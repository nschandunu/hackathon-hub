...
export default function EventGalleryModal({
    ...
    return (
        ...
        >
            ...
            {/* Modal container — fixed 90% screen */}
            <motion.div
                initial={{ scale: 0.92, opacity: 0 }}
                animate={{ scale: 1, opacity: 1 }}
                exit={{ scale: 0.92, opacity: 0 }}
                transition={{ duration: 0.45, ease: appleEase }}
                className="relative w-[90vw] h-[90vh] rounded-2xl overflow-hidden bg-[#0a0a0a] border border-white/[0.06]"
                onClick={(e) => e.stopPropagation()}
            >
                {/* Close button */}
                <button
                    onClick={onClose}
                    className="absolute top-6 right-6 z-50 w-10 h-10 rounded-full bg-black/60 backdrop-blur-md border border-white/10 flex items-center justify-center text-white/70 hover:text-white hover:bg-black/80 transition-all duration-300 cursor-pointer"
                >
                    <svg
                        className="w-5 h-5"
                        fill="none"
                ...
                <div className="overflow-y-auto flex-1 h-full w-full p-6 pb-48">
                ...
                {/* Fixed gradient description overlay */}
                <div className="absolute bottom-0 left-0 w-full pointer-events-none">
                    <div className="bg-gradient-to-t from-black via-black/90 to-transparent pt-32 pb-8 px-8">
                        {/* Status + Date row */}
                        <div className="flex items-center gap-3 mb-3 pointer-events-auto">
                            <span
                                className={`px-2.5 py-1 rounded-full text-[9px] tracking-[0.2em] font-medium backdrop-blur-sm border ${isUpcoming
